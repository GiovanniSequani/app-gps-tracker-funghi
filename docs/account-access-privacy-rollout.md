...
| 2026-09-23 | OPS-005 export-ready same-run dispatch | DONE | La pipeline locale mantiene lifecycle prima di rights e aggiunge un secondo passaggio lifecycle subito dopo rights. Gli export completati accodano `export_ready` atomicamente e vengono inviati nello stesso lancio; riapertura same-day, dedupe, contatori, retry e backoff DB restano invariati. Test fittizio su due esecuzioni prova un solo invio, senza SMTP reale. |
