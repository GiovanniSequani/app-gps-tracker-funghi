...
export function AccountRightsPanel(props: { accountState: AccountState }) {
  ...
  if (props.accountState === 'deletion_pending') return <View style={[styles.panel, styles.pending]} accessibilityRole="summary">
    ...
    <Text style={styles.body}>L’account è già in eliminazione. Percorsi, ritrovamenti, profilo, export temporanei e dati ancora riconducibili all’account vengono rimossi dal backend appena possibile; eventuali residui tecnici entro 30 giorni.</Text>
  ...
  return <View style={styles.panel} accessibilityRole="summary">
    ...
    {props.accountState === 'restricted' && <Text style={styles.limited}>Export ed eliminazione restano disponibili anche con account sospeso. Archivio GPX e altre funzioni riservate rimangono bloccati.</Text>}
    ...
    <View style={styles.section}>
      ...
      {!rights.loading && isPreparing && <View style={styles.delivery} accessibilityLiveRegion="polite"><MailCheck size={16} color={COLORS.amber} /><Text style={styles.deliveryText}>Stiamo preparando il tuo archivio. La notifica email non è ancora attiva: controlla qui lo stato con Aggiorna. Il download avrà una scadenza.</Text></View>}
...
const styles = StyleSheet.create({
  ...
  body: { color: COLORS.muted, fontSize: 12, lineHeight: 19 }, muted: { color: COLORS.muted, fontSize: 11, lineHeight: 17 }, strong: { color: COLORS.text, fontSize: 14, fontWeight: '800' }, limited: { color: COLORS.amber, fontSize: 12, lineHeight: 18 }, warning: { color: COLORS.amber, fontSize: 12, lineHeight: 18 }, error: { color: '#ffaaaa', fontSize: 12, lineHeight: 18 }, success: { color: '#9be9aa', fontSize: 12, lineHeight: 18 },
  ...
  actions: { flexDirection: 'row', gap: 8 }, primary: { flex: 1, minHeight: 43, borderRadius: 8, backgroundColor: COLORS.green, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }, primaryText: { color: '#0a110b', fontSize: 12, fontWeight: '800' }, secondary: { minHeight: 43, borderRadius: 8, paddingHorizontal: 12, borderWidth: 1, borderColor: COLORS.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 }, secondaryText: { color: COLORS.text, fontSize: 12, fontWeight: '800' }, disabled: { opacity: .45 }, note: { flexDirection: 'row', alignItems: 'flex-start', gap: 6 }, noteText: { flex: 1, color: COLORS.amber, fontSize: 11, lineHeight: 16 },
