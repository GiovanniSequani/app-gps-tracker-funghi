...
- Il download di una singola traccia genera in locale un GPX derivato: applica nome, trim e marker correnti riletti dal database, conserva i waypoint storici e non modifica mai il GPX raw privato. Anche questa copia temporanea viene eliminata dopo il foglio di condivisione.
...
- I GPX privati vengono scaricati esclusivamente con `storage.from('user-gpx').download(storage_path)`. Su Android/iOS il `Blob` nativo viene convertito tramite `FileReader`, senza signed URL e senza affidarsi a `Blob.arrayBuffer()`, che non è affidabile nelle build React Native release. Dimensione dichiarata e byte letti devono coincidere prima di parsing, mappa o condivisione.
- La cancellazione mantiene l'ordine Storage poi RPC metadata. La RPC, idempotente, viene ritentata per un numero limitato di volte soltanto in caso di risposta di rete ambigua o breve ritardo di visibilità della cancellazione Storage; la DELETE dell'oggetto non viene ripetuta.
- La diagnostica GPX registra soltanto nome dell'operazione, categoria, tipo e codici tecnici limitati: non registra messaggi originali, URL, JWT, email, UUID, path, coordinate o contenuti GPX.
